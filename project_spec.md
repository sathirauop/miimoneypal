# Project Specification: MiiMoneyPal (MVP)
...
Buckets (Unit Trusts, Car Fund, etc.) record **cash contributions only** — not market value.
...
```
Monthly Usable = Opening Balance + Σ Income + Σ Withdrawals - Σ Expenses - Σ Investments
```
...
```
Monthly Savings Flow = Σ Investments (This Month) - Σ Withdrawals (This Month)
```

**Bucket Balance:** Tracks total cash contributed to a specific Goal/Asset.

```
Bucket Balance = Σ (All Investments to Bucket) - Σ (All Withdrawals from Bucket)
```
...
|------|------------|---------|-------------------------|--------------------------|
| GOAL_COMPLETED | System ("Mark as Spent" action) | Zero out a savings goal | Increases (+) | Decreases to 0 |
...
Implementation:
1. System creates a `GOAL_COMPLETED` withdrawal transaction equal to current bucket balance
2. Transaction note auto-populated: "Goal completed: {bucket_name}"
3. Bucket status set to `ARCHIVED`
4. Bucket balance becomes 0 (via the withdrawal)
5. Archived buckets visible in a separate "Completed Goals" section

**Confirmation:** User must confirm action via a prompt.

**Reversal: "Reopen Goal"**
1. User taps "Reopen" on archived goal
2. Prompt: "Enter amount to restore to this goal"
...
- Archived items accessible via Settings → "Manage Archived Items"
...
- Completed Goals (Archived SAVINGS_GOAL) — collapsible section
...
**Date Boundaries:** Strict Calendar Month (1st–last day of month).
...
├── architecture/                           (Core architecture interfaces)
│   ├── UseCase.java                        (Base use case interface)
│   └── AuthenticatedUseCase.java           (For authenticated operations)
├── rest/                                   (The Core - Vertical Slices)
│   ├── WelcomeController.java              (Root controller)
...
│   ├── AppUser.java                        (UserDetails implementation)
│   ├── Role.java                           (Role enum)
│   └── jwt/                                (JWT specific logic)
├── config/                                 (Spring configurations)
│   ├── SecurityConfig.java
│   ├── WebConfig.java                      (CORS, Jackson settings)
│   └── JwtProperties.java                  (@ConfigurationProperties)
├── exception/                              (Global Error Handling)
│   ├── GlobalExceptionHandler.java
│   └── response/                           (Error DTOs)
├── models/                                 (Common Response wrappers)
│   └── response/
│       ├── ApiResponse.java                (Marker interface)
│       └── OffsetSearchResponse.java       (Pagination wrapper)
├── records/                                (Domain Entities & DTOs)
│   ├── transaction/
│   ├── category/
│   ├── bucket/
│   └── user/
├── repository/                             (Global/Shared Repositories)
│   └── UserRepository.java
├── service/                                (Shared Services)
│   └── CategorySeederService.java          (Seeds default categories on registration)
└── constants/
    └── EndPoints.java                      (Centralized URL paths)
```
...
- **Global Repository (10%):** Lives in `repository/`. Shared across multiple features. **Criteria:** If 3+ endpoints need the same query → Global Repository.
...
- **Category Dropdown:** Shown only for Income/Expense types (filtered by type)
- **Bucket Dropdown:** Shown only for Investment/Withdrawal types
- **Note:** Optional text input, max 255 characters

**Actions:**
- **Submit:** POST to API → invalidate TanStack Query cache → close drawer
...
- Add (+) — icon: `plus` (center, prominent, fab-style)
- Buckets — icon: `piggy-bank`
- Settings — icon: `cog`

### E. MonthRolloverPrompt

**Trigger:** First app open when current month ≠ last active month AND no opening balance exists for current month.
...
│  Start January 2025                     │
│                                         │
│  Your December ending balance was:      │
│  LKR 45,000                             │
│                                         │
...
GET    /categories
       Response: { data: [...] }
       Notes: Returns only active (non-archived) categories
...
DELETE /categories/{id}
       Response: { success: true } OR { error: "has_transactions", transaction_count: 5 }
...
GET    /buckets
       Response: { data: [...] }
       Notes: Includes current_balance calculated field

POST   /buckets
       Request:  { name, type, target_amount? }
       Response: { id, name, type, target_amount, current_balance, created_at }
...
POST   /buckets/{id}/reopen
       ...
       Notes: Creates Investment transaction, un-archives bucket
```
...
│   │   ├── Dashboard.tsx
│   │   └── components/
│   │       ├── UsableAmountCard.tsx
│   │       ├── MonthlyStatsChart.tsx
│   │       ├── RecentActivity.tsx
│   │       ├── MonthPicker.tsx
│   │       ├── MonthRolloverPrompt.tsx
│   │       └── WelcomePrompt.tsx
│   │
│   ├── transactions/
│   │   ├── api.ts
...
- ❌ **DON'T** create "God" services (e.g., TransactionService with 20 methods)
- ✅ **DO** create focused UseCases (e.g., PostTransactionUseCase)

- ❌ **DON'T** inject Repositories into Controllers
- ✅ **DO** inject UseCases into Controllers

- ❌ **DON'T** build Response DTOs manually in UseCases
- ✅ **DO** delegate to Presenters

- ❌ **DON'T** use @Transactional on read-only methods
- ✅ **DO** use @Transactional only for write operations

- ❌ **DON'T** hardcode URLs in code
- ✅ **DO** use EndPoints constants
